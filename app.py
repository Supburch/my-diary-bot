...
@app.get("/debug/test-streak")
async def debug_test_streak():
    """[TEMP DEBUG] สั่งรัน Unit Test 8 เคสของ Streak Tracking บนสภาพแวดล้อม Render จริง"""
    import unittest
    import io
    from tests.test_streak import TestStreakCalculation
    
    suite = unittest.TestLoader().loadTestsFromTestCase(TestStreakCalculation)
    stream = io.StringIO()
